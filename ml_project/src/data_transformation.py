...
def build_preprocessor(df):
    """Transform data by scaling numerical features and encoding catgeorical features.
    """
    # select numerical and categorical features excluding target column
    numeric_featires = df.drop(columns=[TARGET_COL]).select_dtypes(include=['number']).columns.tolist()

    categorical_features = df.drop(columns = [TARGET_COL]).select_dtypes(include=['object', 'category']).columns.tolist()

    preprocessor = ColumnTransformer(
        transformers=[
            ('num', StandardScaler(), numeric_featires),
